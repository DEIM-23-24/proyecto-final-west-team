...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, (0.792 /*_RimWidth*/));
    #ifdef POI_AUDIOLINK
    ...
    rim = 1 - smoothstep(min((0.438 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(1,1,1,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, (0.57 /*_RimLightColorBias*/)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, (0.57 /*_RimLightColorBias*/)) + lerp(albedo.rgb, rimColor, (0.57 /*_RimLightColorBias*/)) * rimBrighten, rim);
}
#endif
