...
float3 getDirectSpecular(float roughness, float ndh, float vdn, float ndl, float ldh, float3 f0, float3 halfVector, float3 tangent, float3 bitangent, float anisotropy)
{
        ...
        return lerp(directSpecularNonAniso, directSpecularAniso, saturate(abs((0.82 /*_BRDFAnisotropy*/) * 100))) * 3; // * 100 to prevent blending, blend because otherwise tangents are fucked on lightmapped object
    #else
        return 0;
    #endif
}
...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    float metallic = metallicGlossMap.r * (0.518 /*_BRDFMetallic*/);
    float reflectance = metallicGlossMap.g * (0.5 /*_BRDFReflectance*/);
    float roughness = max(1 - ((0.0 /*_BRDFGlossiness*/) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
    finalColor.rgb *= lerp(1, 1 - metallic, (1.0 /*_BRDFReflectionsEnabled*/));
    float3 reflViewDir = getAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent.xyz, poiMesh.normals[1], roughness, (0.82 /*_BRDFAnisotropy*/));
    ...
    float3 directSpecular = getDirectSpecular(roughness, saturate(poiLight.nDotH), max(poiLight.nDotV, 0.000001), attenuation, saturate(poiLight.lDotH), f0, poiLight.halfDir, poiMesh.tangent.xyz, poiMesh.binormal, (0.82 /*_BRDFAnisotropy*/)) * poiLight.attenuation * attenuation * poiLight.color;
    directSpecular = min(directSpecular, poiLight.color);
    float3 vDirectSpecular = 0;
    #ifdef VERTEXLIGHT_ON
        for (int index = 0; index < 4; index++)
        {
            float3 v0directSpecular = getDirectSpecular(roughness, saturate(poiLight.vDotNH[index]), max(poiLight.nDotV, 0.000001), attenuation, saturate(poiLight.lDotH), f0, poiLight.vHalfDir[index], poiMesh.tangent, poiMesh.binormal, (0.82 /*_BRDFAnisotropy*/)) * poiLight.attenuation * poiLight.vAttenuationDotNL[index] * poiLight.vColor[index];
