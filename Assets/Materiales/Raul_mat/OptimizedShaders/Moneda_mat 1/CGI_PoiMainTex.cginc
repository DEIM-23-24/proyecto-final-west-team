...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        albedo = float4(mainTexture.rgb * max(float4(0.7615293,0.3696578,0.03741179,1).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, vertexColor, (0.0 /*_MainVertexColoring*/)), mainTexture.a * max(float4(0.7615293,0.3696578,0.03741179,1).a, 0.0000001));
        ...
        #ifdef COLOR_GRADING_HDR
            #if defined(PROP_MAINCOLORADJUSTTEXTURE) || !defined(OPTIMIZER_ENABLED)
                float4 hueShiftAlpha = POI2D_SAMPLER_PAN(_MainColorAdjustTexture, _MainTex, poiMesh.uv[(0.0 /*_MainColorAdjustTextureUV*/)], float4(0,0,0,0));
            #else
                float4 hueShiftAlpha = 1;
            #endif
            if ((1.0 /*_MainHueShiftReplace*/))
            {
                albedo.rgb = lerp(albedo.rgb, hueShift(albedo.rgb, mixedHueShift + (0.0 /*_MainHueShiftSpeed*/) * _Time.x), hueShiftAlpha.r);
            }
            else
            {
                albedo.rgb = hueShift(albedo.rgb, frac((mixedHueShift - (1 - hueShiftAlpha.r) + (0.0 /*_MainHueShiftSpeed*/) * _Time.x)));
            }
            albedo.rgb = lerp(albedo.rgb, dot(albedo.rgb, float3(0.3, 0.59, 0.11)), -(0.0 /*_Saturation*/) * hueShiftAlpha.b);
            albedo.rgb = saturate(albedo.rgb + (0.0 /*_MainBrightness*/) * hueShiftAlpha.g);
        #endif
