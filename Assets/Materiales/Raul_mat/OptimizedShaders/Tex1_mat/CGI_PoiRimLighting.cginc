...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, (0.624 /*_RimWidth*/));
    #ifdef POI_AUDIOLINK
    ...
    rim = 1 - smoothstep(min((0.839 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0,0,0,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, (0.45 /*_RimLightColorBias*/)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, (0.45 /*_RimLightColorBias*/)) + lerp(albedo.rgb, rimColor, (0.45 /*_RimLightColorBias*/)) * rimBrighten, rim);
}
#endif
