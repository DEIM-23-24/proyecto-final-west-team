...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimStrength = (2.1 /*_RimStrength*/);
    float rimBrighten = (0.0 /*_RimBrighten*/);
    float rimWidth = lerp( - .05, 1, (0.483 /*_RimWidth*/));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[(0.0 /*_RimTexUV*/)], float4(0,0,0,0)) * float4(0.006628771,0.005987724,0.005753471,1);
    #else
        rimColor = float4(0.006628771,0.005987724,0.005753471,1);
    #endif
    ...
    rim *= float4(0.006628771,0.005987724,0.005753471,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, (0.703 /*_RimLightColorBias*/)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, (0.703 /*_RimLightColorBias*/)) + lerp(albedo.rgb, rimColor, (0.703 /*_RimLightColorBias*/)) * rimBrighten, rim);
}
#endif
