...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimStrength = (2.2 /*_RimStrength*/);
    float rimBrighten = (0.0 /*_RimBrighten*/);
    float rimWidth = lerp( - .05, 1, (0.756 /*_RimWidth*/));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[(0.0 /*_RimTexUV*/)], float4(0,0,0,0)) * float4(0.6070304,0.2103922,0.002049335,1);
    #else
        rimColor = float4(0.6070304,0.2103922,0.002049335,1);
    #endif
    ...
    rim = 1 - smoothstep(min((0.451 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0.6070304,0.2103922,0.002049335,1).a * rimColor.a * rimMask;
