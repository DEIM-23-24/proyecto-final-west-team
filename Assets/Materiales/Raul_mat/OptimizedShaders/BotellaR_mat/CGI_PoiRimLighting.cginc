...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, (0.23 /*_RimWidth*/));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[(0.0 /*_RimTexUV*/)], float4(0,0,0,0)) * float4(0.001460365,0.001460365,0.001460365,1);
    #else
        rimColor = float4(0.001460365,0.001460365,0.001460365,1);
    #endif
    ...
    rim = 1 - smoothstep(min((0.326 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0.001460365,0.001460365,0.001460365,1).a * rimColor.a * rimMask;
