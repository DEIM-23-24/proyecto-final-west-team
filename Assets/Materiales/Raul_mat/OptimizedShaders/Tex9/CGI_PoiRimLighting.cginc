...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[(0.0 /*_RimTexUV*/)], float4(0,0,0,0)) * float4(0,0,0,1);
    #else
    ...
    rimWidth = max(lerp(rimWidth, rimWidth * lerp(0, 1, poiLight.lightMap - (0.0 /*_ShadowMixThreshold*/)) * (1.7 /*_ShadowMixWidthMod*/), (0.085 /*_ShadowMix*/)), 0);
    rim = 1 - smoothstep(min((0.0 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0,0,0,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, (0.727 /*_RimLightColorBias*/)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, (0.727 /*_RimLightColorBias*/)) + lerp(albedo.rgb, rimColor, (0.727 /*_RimLightColorBias*/)) * rimBrighten, rim);
}
#endif
