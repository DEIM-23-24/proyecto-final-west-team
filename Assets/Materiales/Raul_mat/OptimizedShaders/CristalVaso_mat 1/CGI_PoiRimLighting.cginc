...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
        ...
        float rimNoise = 0;
    #endif
    rimNoise = (rimNoise - .5) * (0.662 /*_RimWidthNoiseStrength*/);
    ...
    float rimStrength = (20.0 /*_RimStrength*/);
    float rimBrighten = (3.0 /*_RimBrighten*/);
    float rimWidth = lerp( - .05, 1, (0.564 /*_RimWidth*/));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[(0.0 /*_RimTexUV*/)], float4(0,0,0,0)) * float4(1,1,1,1);
    #else
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, (0.201 /*_RimLightColorBias*/)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, (0.201 /*_RimLightColorBias*/)) + lerp(albedo.rgb, rimColor, (0.201 /*_RimLightColorBias*/)) * rimBrighten, rim);
}
#endif
