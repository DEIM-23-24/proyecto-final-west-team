...
void applyBackFaceTexture(inout float backFaceDetailIntensity, inout float mixedHueShift, inout float4 albedo, inout float3 backFaceEmission)
{
	...
	if ((0.0 /*_BackFaceEnabled*/))
	{
		...
		{
			#if defined(PROP_BACKFACETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 backFaceTex = POI2D_SAMPLER_PAN(_BackFaceTexture, _MainTex, poiMesh.uv[(0.0 /*_BackFaceTextureUV*/)], float4(0,0,0,0)) * float4(0,0,0,1);
			#else
				float4 backFaceTex = float4(0,0,0,1);
			#endif
			...
			backFaceDetailIntensity = (1.4 /*_BackFaceDetailIntensity*/);
			BackFaceColor = albedo.rgb;
			mixedHueShift = (0.0 /*_BackFaceHueShift*/);
			backFaceEmission = BackFaceColor * (10.82 /*_BackFaceEmissionStrength*/);
		}
	}
}
#endif
