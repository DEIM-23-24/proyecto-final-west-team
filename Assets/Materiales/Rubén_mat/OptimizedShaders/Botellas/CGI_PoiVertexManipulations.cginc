    ...
    {
        #if defined(PROP_VERTEXMANIPULATIONHEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
            float3 heightOffset = (tex2Dlod(_VertexManipulationHeightMask, float4(TRANSFORM_TEX(uv, _VertexManipulationHeightMask) + float4(0,0,0,0) * _Time.x, 0, 0)).r - (0.522 /*_VertexManipulationHeightBias*/)) * (0.0 /*_VertexManipulationHeight*/) * worldNormal;
        #else
    ...
    void applyVertexRounding(inout float4 worldPos, inout float4 localPos)
    {
        ...
        {
            worldPos.xyz = (ceil(worldPos.xyz * (0.02 /*_VertexRoundingDivision*/)) / (0.02 /*_VertexRoundingDivision*/)) - 1 / (0.02 /*_VertexRoundingDivision*/) * .5;
