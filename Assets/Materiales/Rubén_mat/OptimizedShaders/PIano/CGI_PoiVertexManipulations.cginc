    ...
    {
        #if defined(PROP_VERTEXMANIPULATIONHEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
            float3 heightOffset = (tex2Dlod(_VertexManipulationHeightMask, float4(TRANSFORM_TEX(uv, _VertexManipulationHeightMask) + float4(0,0,0,0) * _Time.x, 0, 0)).r - (0.0 /*_VertexManipulationHeightBias*/)) * (0.08 /*_VertexManipulationHeight*/) * worldNormal;
        #else
            float3 heightOffset = (0.08 /*_VertexManipulationHeight*/) * worldNormal;
        #endif
    ...
    void applyVertexRounding(inout float4 worldPos, inout float4 localPos)
    {
        ...
        {
            worldPos.xyz = (ceil(worldPos.xyz * (0.0 /*_VertexRoundingDivision*/)) / (0.0 /*_VertexRoundingDivision*/)) - 1 / (0.0 /*_VertexRoundingDivision*/) * .5;
