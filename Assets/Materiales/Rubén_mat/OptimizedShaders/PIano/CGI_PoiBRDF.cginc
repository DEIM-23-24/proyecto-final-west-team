...
float3 getDirectSpecular(float roughness, float ndh, float vdn, float ndl, float ldh, float3 f0, float3 halfVector, float3 tangent, float3 bitangent, float anisotropy)
{
        ...
        return lerp(directSpecularNonAniso, directSpecularAniso, saturate(abs((-1.0 /*_BRDFAnisotropy*/) * 100))) * 3; // * 100 to prevent blending, blend because otherwise tangents are fucked on lightmapped object
    #else
        return 0;
    #endif
}
...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    float metallic = metallicGlossMap.r * (0.67 /*_BRDFMetallic*/);
    float reflectance = metallicGlossMap.g * (0.195 /*_BRDFReflectance*/);
    float roughness = max(1 - ((0.484 /*_BRDFGlossiness*/) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
    finalColor.rgb *= lerp(1, 1 - metallic, (1.0 /*_BRDFReflectionsEnabled*/));
    float3 reflViewDir = getAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent.xyz, poiMesh.normals[1], roughness, (-1.0 /*_BRDFAnisotropy*/));
