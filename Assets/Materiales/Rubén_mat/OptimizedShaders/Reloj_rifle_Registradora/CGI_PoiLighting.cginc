...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        {
            occlusion = lerp(1, POI2D_SAMPLER_PAN(_LightingAOTex, _MainTex, poiMesh.uv[(0.0 /*_LightingAOTexUV*/)], float4(0,0,0,0)).r, (0.298 /*_AOStrength*/));
        }
...
void calculateBasePassLightMaps()
{
        ...
        switch((0.0 /*_LightingRampType*/))
        {
            case 0: // Ramp Texture
            {
                poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + (-0.05 /*_ShadowOffset*/)).rgb, shadowStrength.r);
...
float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
{
    ...
    {
        return lerp(lightColor * attenuation, lightColor * (0.529 /*_LightingAdditivePassthrough*/) * attenuation, smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.587 /*_LightingAdditiveGradientEnd*/), dotNL)) * detailShadow;
    }
...
float3 calculateFinalLighting(inout float3 albedo, float4 finalColor)
{
    float3 finalLighting = 1;
        ...
        else if ((1.0 /*_LightingAdditiveType*/) == 1) // Toon
        {
                ...
                finalLighting = lerp(poiLight.color * max(poiLight.additiveShadow, (0.529 /*_LightingAdditivePassthrough*/)), poiLight.color * (0.529 /*_LightingAdditivePassthrough*/), smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.587 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation * detailShadow;
            #else
                finalLighting = lerp(poiLight.color * max(poiLight.attenuation, (0.529 /*_LightingAdditivePassthrough*/)), poiLight.color * (0.529 /*_LightingAdditivePassthrough*/), smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.587 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * detailShadow;
            #endif
        }
        else //if((1.0 /*_LightingAdditiveType*/) == 2) // Wrapped
        {
            ...
            if ((0.0 /*_LightingRampType*/) == 1) // Math Gradient
            poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (0.5 /*_LightingGradientEnd*/), 1 - uv)));
            float shadowatten = max(poiLight.additiveShadow, (0.529 /*_LightingAdditivePassthrough*/));
